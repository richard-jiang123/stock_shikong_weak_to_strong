...
**逐日判断，最早触发**。每个持仓日检查当日最低价是否跌破止损价。
如果跌破，以止损价出场（模拟盘中实际能成交的价格）。

**举例**:
```
回调阶段最低价 = 100.00 元
止损价 = 100.00 × 0.98 = 98.00 元
买入价 = 105.00 元
某日最低价触及 97.50 元 → 触发止损 → 出场价 98.00 元
亏损 = (98.00 - 105.00) / 105.00 = -6.67%
```

**效果**: 19.2% 的交易由此退出，胜率仅 5.6%，平均亏损 -10.88%。
**为什么必要**: 回调最低点被跌破，说明"弱转强"判断失效，必须立即离场。

...
| 文件 | 说明 |
|---|---|
| `daily_run.sh` | **每日自动化入口**: 支持 `--date` 参数指定扫描日期，日志输出带时间戳、运行耗时统计、颜色输出 |
| `data_layer.py` | **数据层**: SQLite缓存管理，增量更新，批量查询，断点续传，数据完整性检查，行业分类 |
| `data_source.py` | **多数据源支持**: baostock主数据源 + 腾讯实时行情备份，盘中数据补充 |
| `process_lock.py` | **进程锁**: 文件锁机制，防止多实例同时运行关键任务（batch_update等） |
| `daily_scanner.py` | **每日选股**: 增量更新 + 本地扫描，输出 `YYYYMMDD_today_signals.xlsx`（含行业分类），自动记录选股用于跟踪，颜色输出 |
| `strategy_config.py` | **参数中心**: 所有策略参数集中管理，DB 存储/读取，替代硬编码 CONFIG |
| `pick_tracker.py` | **选股跟踪**: 记录每日选股、模拟后续表现（止损/止盈/时间退出）、生成成绩单，中文/英文表头兼容，颜色盈亏显示 |
...
| 文件 | 说明 |
|---|---|
| `stock_data.db` | SQLite 数据库 (~140MB，包含全市场K线 + 策略参数 + 选股跟踪 + 行业分类) |
...
```
shikong_fufei/
├── daily_run.sh                 # 每日自动化流程入口（支持--date参数 + 耗时统计 + 颜色输出）
├── data_layer.py                # 数据层（增量更新+批量查询+断点续传+行业分类+数据完整性检查）
├── data_source.py               # 多数据源支持（baostock + 腾讯实时行情）
├── process_lock.py              # 进程锁（防止多实例并发）
├── daily_scanner.py             # 每日选股（含自动跟踪 + XLSX输出 + 行业分类 + 颜色输出）
├── strategy_config.py           # 参数中心
├── pick_tracker.py              # 选股跟踪模型（中文/英文表头兼容 + 颜色盈亏显示）

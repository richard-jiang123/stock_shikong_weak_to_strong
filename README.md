...
| 文件 | 说明 |
|---|---|
| `daily_run.sh` | **每日自动化入口**: 支持 `--date` 参数指定扫描日期，日志输出带时间戳、运行耗时统计、颜色输出。新增 `--monitor`、`--weekly-optimize`、`--adaptive`、`--status` 选项 |
...
| `sandbox_validator.py` | **沙盒验证**: 滚动窗口验证（3周），防止一次性调整风险 |
| `adaptive_engine.py` | **自适应引擎**: 核心控制器，调度每日监控 + 每周优化 |
...
```
shikong_fufei/
├── daily_run.sh                 # 每日自动化流程入口（新增--monitor/--weekly-optimize/--adaptive/--status选项）
├── data_layer.py                # 数据层（新增signal_status/optimization_history/market_regime/daily_monitor_log表）
├── data_source.py               # 多数据源支持（baostock + 腾讯实时行情）
├── process_lock.py              # 进程锁（防止多实例并发）
├── daily_scanner.py             # 每日选股（新增score_*评分明细字段）
├── strategy_config.py           # 参数中心（新增DYNAMIC_PARAMS动态参数）
├── pick_tracker.py              # 选股跟踪模型（新增score_*字段）
...
├── weekly_optimizer.py          # 每周四层优化调度
├── sandbox_validator.py         # 沙盒验证（滚动窗口3周）
├── adaptive_engine.py           # 自适应引擎核心控制器
